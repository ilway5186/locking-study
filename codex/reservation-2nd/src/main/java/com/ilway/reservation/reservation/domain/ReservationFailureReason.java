...
public enum ReservationFailureReason {
  ...
  GROUP_NOT_FOUND(HttpStatus.NOT_FOUND, "예약 그룹을 찾을 수 없습니다."),
  BOOKING_NOT_OPEN(HttpStatus.BAD_REQUEST, "예매 오픈 전입니다."),
  BOOKING_CLOSED(HttpStatus.BAD_REQUEST, "예매가 마감되었습니다."),
  INVALID_SEAT_SELECTION(HttpStatus.BAD_REQUEST, "잘못된 좌석 선택입니다."),
  SEAT_ALREADY_HELD(HttpStatus.CONFLICT, "이미 다른 요청이 좌석을 HOLD 중입니다."),
  SEAT_PAYMENT_PENDING(HttpStatus.CONFLICT, "이미 다른 요청이 결제를 진행 중인 좌석입니다."),
  ...
  FORBIDDEN_GROUP_ACCESS(HttpStatus.FORBIDDEN, "다른 사용자의 예약 그룹에는 접근할 수 없습니다."),
  ...
  INVALID_GROUP_STATE(HttpStatus.CONFLICT, "현재 예약 그룹 상태에서는 요청을 수행할 수 없습니다."),
